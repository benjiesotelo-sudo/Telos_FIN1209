#!/usr/bin/env python3
"""Build the FIN1209 Chapter 3 lecture deck and the instructor answer sheet.
...
produces from build/content_chapter03.py.
...
Both editions first draw the six Chapter 3 teaching charts from
build/charts_chapter03.py, so a fresh clone produces the real slide rather
...
This script is build_chapter2.py with the names changed. Chapter 4 is the
same move again: copy it, point it at content_chapter04.py, and the same
helpers in deckkit.py render it. See TEMPLATE.md.
"""
...
DECK_OUT = REPO / "chapter-03" / "FIN1209-Chapter-03.pptx"
STUDENT_OUT = REPO / "chapter-03" / "FIN1209-Chapter-03-Student-Edition.pptx"
CHECKS_OUT = REPO / "chapter-03" / "in-class-checks.md"
...
# See chapter-03/README.md for the two commands.
...
# and a shared folder would have Chapter 3's Chart A overwrite another chapter's.
...
def write_checks(chapter, path: Path, total_checks: int) -> None:
    ...
    out.append(
        ...
        "It is generated by `build/build_chapter3.py` from "
        "`build/content_chapter03.py`, so it always matches the deck."
    )
    out.append("")
    out.append(
...
def main() -> int:
    ...
    if figures_dir is not None and not figures_dir.is_dir():
        parser.error(
            ...
            "repository; see chapter-03/README.md. Drop the flag to build "
