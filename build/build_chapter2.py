#!/usr/bin/env python3
"""Build the FIN1209 Chapter 2 lecture deck and the instructor answer sheet.
...
produces from build/content_chapter02.py.
...
Both editions first draw the eight Chapter 2 teaching charts from
build/charts_chapter02.py, so a fresh clone produces the real slide rather
...
This script is build_chapter1.py with the names changed. Chapter 3 is the
same move again: copy it, point it at content_chapter03.py, and the same
...
DECK_OUT = REPO / "chapter-02" / "FIN1209-Chapter-02.pptx"
STUDENT_OUT = REPO / "chapter-02" / "FIN1209-Chapter-02-Student-Edition.pptx"
CHECKS_OUT = REPO / "chapter-02" / "in-class-checks.md"
...
# See chapter-02/README.md for the two commands.
...
# and a shared folder would have Chapter 2's Chart A overwrite Chapter 1's.
...
def write_checks(chapter, path: Path, total_checks: int) -> None:
    ...
    out.append(
        ...
        "It is generated by `build/build_chapter2.py` from "
        "`build/content_chapter02.py`, so it always matches the deck."
    )
    out.append("")
    out.append(
...
def main() -> int:
    ...
    if figures_dir is not None and not figures_dir.is_dir():
        parser.error(
            ...
            "repository; see chapter-02/README.md. Drop the flag to build "
