#!/usr/bin/env python3
"""Build the FIN1209 Chapter 3 student lecture notes as a print-ready PDF.
...
Content is data in build/lecture_chapter03.py. Layout is build/lecturekit.py,
...
The teaching copy, with the artwork, goes outside the repository:
    ...
    .venv/bin/python build/build_lecture_notes3.py --with-figures \\
        --out ~/FIN1209-Chapter-03-Lecture-Notes.pdf

After building, look at the result. Every page:

    DATA=/Users/benjie/benjie-agent-workspace/data/fin1209-notes-rebuild
    $DATA/pdfpng chapter-03/FIN1209-Chapter-03-Lecture-Notes.pdf /tmp/ln \\
        $(seq 1 28)
...
from charts_chapter03 import CHARTS  # noqa: E402
from content_chapter03 import CHAPTER  # noqa: E402
from lecture_chapter03 import NOTES  # noqa: E402

REPO = Path(__file__).resolve().parent.parent
PDF_OUT = REPO / "chapter-03" / "FIN1209-Chapter-03-Lecture-Notes.pdf"
FIGURES_DIR = REPO / "assets" / "figures"

# The charts this course drew are ours, so unlike the figures they are
# generated here on every build and placed in the committed PDF too. The
# folder is gitignored because it is output. See build/chartkit.py.
CHARTS_DIR = REPO / "build" / "generated" / "charts-03"


def deck_facts(chapter) -> tuple[dict[str, str], dict[str, str],
    ...
    for section in chapter.sections:
        for slide in section.slides:
            if isinstance(slide, deckkit.Figure):
                ...
                chart_shows[slide.letter] = slide.shows
                chart_files[slide.letter] = slide.filename
            elif isinstance(slide, deckkit.Term):
                terms.append(slide.term)
    return shows, files, chart_shows, chart_files, terms


# The two closing slides the notes reproduce verbatim, by title.
SUMMARY_SLIDE = "Chapter 3 in five sentences"
...
def deck_closing(chapter) -> tuple[tuple[str, ...], tuple[str, ...]]:
    ...
    if missing:
        raise SystemExit(
            ...
            "and in build_lecture_notes3.py together."
        )
...
def main() -> int:
    ...
    if args.with_figures and not args.figures_dir.is_dir():
        parser.error(
            ...
            "repository; see chapter-03/README.md. Drop the flag to build the "
            "placeholder version."
        )
    ...
    shows, files, chart_shows, chart_files, terms = deck_facts(CHAPTER)
    summary, review = deck_closing(CHAPTER)

    # Drawn before the document, and in every build, so a clone that has
    # never built anything still gets the real chart in the student's copy.
    chartkit.generate(CHARTS, CHARTS_DIR)
    ...
    figures = lecturekit.FigureFacts(
        ...
        chart_directory=CHARTS_DIR,
        # Written once, in deckkit, so the slide and the page cannot disagree.
        chart_credit_line=deckkit.CHART_CREDIT,
    )
    ...
    problems = lecturekit.validate(document, notes, shows, terms, chart_shows)
    if problems:
        raise SystemExit(
    ...
    print(f"figures: {len(notes.figure_numbers())} placed, "
          f"artwork {placed} of {len(notes.figure_numbers())}")
    print(f"charts : {len(notes.figure_numbers('chart'))} placed, artwork "
          f"{figures.charts_placed()} of {len(chart_files)}, generated by "
          f"this build")
