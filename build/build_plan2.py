#!/usr/bin/env python3
"""Build the FIN1209 Chapter 2 run card as a print-ready PDF.

    .venv/bin/python build/build_plan2.py

This is the instructor's document. The student-facing lecture notes are a
separate artifact, built by build/build_lecture_notes2.py.

Content is data in build/plan_chapter02.py. Layout is build/notekit.py, which
...
Three pages, not twenty six. Chapter 1 shipped a full teaching plan and the
instructor did not use it; what he needed was pacing. See the docstring of
...
After building, look at the result. Every page:
    ...
    $DATA/pdfpng chapter-02/FIN1209-Chapter-02-Run-Card.pdf /tmp/card \
        $(seq 1 3)
...
from plan_chapter02 import PLAN  # noqa: E402

REPO = Path(__file__).resolve().parent.parent
PDF_OUT = REPO / "chapter-02" / "FIN1209-Chapter-02-Run-Card.pdf"
...
def main() -> int:
    ...
    print(f"card   : {args.out.relative_to(REPO)}")
    print(f"pages  : {pages}")
